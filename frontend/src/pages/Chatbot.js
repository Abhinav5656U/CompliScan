...
const Chatbot = () => {
  ...
  const startChat = async (nextLanguage = language) => {
    try {
      const response = await api.post('/chatbot/start', { language: nextLanguage });
      setLanguage(nextLanguage);
      if (response.data?.greeting) {
        setMessages([{ role: 'assistant', text: response.data.greeting }]);
      } else {
        setMessages([]);
      }
      setSummary('');
  ...
  const loadComplaints = async () => {
    try {
      ...
      // silently handle if unauthorized
    }
  };
